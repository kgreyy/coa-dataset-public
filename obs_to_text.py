...
DEST = 'pdfs/'
TXT_PATH = 'txt/'
AUD_DOC = re.compile('(Observation|Obsevations)|(Audit[ _]*Report|aar)|(Management[ _]*Letter|ML)') # (Audit[ _]*Report)|(ML)|(Management[ _]*Letter)
ABS_PATH = os.getcwd()
...
def parse2txt_worker(comnfile):
    ...
    if len(filentypes)==0:
        return str(os.listdir(src))

    for file, ftype in filentypes:
        try:
            file_ext = file.rsplit('.', maxsplit=1)[-1]
            return HANDLERS.get(file_ext)(src+file, dest+'observations.txt', ftype=ftype)
        except Exception as e:
    ...
    if len(os.listdir(src))==1:
        ...
        file_ext = file.rsplit('.', maxsplit=1)[-1]
        return HANDLERS.get(file_ext)(src+file, dest+'observations.txt', ftype=-1)
...
if __name__=="__main__":
    ...
    with open('file2txtlog-obs.json', 'w', encoding='utf-8') as f:
        json.dump(outjson, f)
